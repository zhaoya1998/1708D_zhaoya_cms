...
<body style="background: url(/resource/images/back_login.jpg)">
		...
		<form id="form1">
			<div class="form-group form-inline">
				...
				举报类型:<select name="typename" class="form-control" id="typename">
					...
					value="${complainVO.endNum}"><a style="color: red">次数大于0小于60</a>
	...
	<table class="table table-bordered table-hover">
		<tr align="center">
			...
			<td><a href="javascript:myOrder('complain_num')">举报数量<%--  <select
					name="orderMethod">
			...
			<tr align="center">
				...
				<td><c:if test="${u.article.status==1}">
						<button type="button" class="btn btn-success"
							onclick="update(${u.article.id},this)">正常</button>

					</c:if> <c:if test="${u.article.status==2}">
						<button type="button" class="btn btn-warning"
							onclick="update(${u.article.id},this)">停用</button>

					</c:if><a href="/admin/article/look?id=${u.id }"
